...
with tab3:
    ...
    <div class="hero" style="border-left-color:#22c55e">
      ...
      <div style="display:flex;align-items:center;gap:1.5rem;margin-top:0.75rem;flex-wrap:wrap">
        <div style="font-size:0.75rem;color:#64748b">
          Currently filtering: <strong style="color:#f0f4f8">{len(tickers)} tickers screened</strong>
          &rarr; <strong style="color:#48bb78">{n_under_global} undervalued</strong>
          &rarr; optimizer input
        </div>
      </div>
    ...
    # Read DIRECTLY from the `undervalued` variable computed this run (not session_state).
    # session_state["undervalued"] always lags one run behind — using the live variable
    # ensures the optimizer IMMEDIATELY reflects any WACC or Growth slider change.
    opt_tickers = undervalued  # <-- reactive: always the current run's filtered list
