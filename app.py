    ...
    for ticker in tickers:
        try:
            ...
            rows.append({
                ...
                "Debt/Equity":     round(de / 100, 2) if de    else None,
                "Market Cap ($B)": round(mktcap / 1e9, 2) if mktcap else None,
                "Beta":            round(beta, 2)    if beta   else None,
                "Div Yield (%)":   round(div_yld * 100, 2) if div_yld else None,
            })
        except Exception:
            rows.append({"Ticker": ticker, "Name": ticker, "Sector": "Error",
...
with st.sidebar:
    ...
    • WACC: {wacc*100:.2f}% · MoS: {margin_of_safety*100:.0f}%<br>
    • Growth: {growth_rate*100:.1f}% · Terminal g: {terminal_growth*100:.2f}%<br>
    • RF Rate: {risk_free*100:.2f}%<br>
    • Max Wt: {max_weight_pct}% · History: {history_years}yr
    </div>
    """, unsafe_allow_html=True)
...
dcf_inputs = fetch_dcf_inputs(tickers)   # cached — zero extra network cost
